...
(define (push-back q x)
  (when (not x)
    (displayln (format "in pushback with x=~a and q=~a" x q)))
  (if (empty-deque? q)
      (deque empty (list x))
...
(define (push-front q x)
  (when (not x)
    (displayln (format "in pushfront with x=~a and q=~a" x q)))
...
(define (count q)
  #;(when (and (empty? (deque-in q)) (= 1 (length (deque-out q))))
    (displayln (format "in count for deque ~a  ~a"  (length (deque-in q)) (length (deque-out q))))
    (displayln q))
  (+ (length (deque-in q)) 
     (length (deque-out q))))
